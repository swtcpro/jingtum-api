...
function WsServer(server, wsPort, host, callback) {
    this.remote = remote;
    ...
    if(server){
        ...
        const ser = new https.createServer({
            cert: server.cert,
            key: server.key,
            rejectUnauthorized: false,
            // ca: server.ca
        }).listen(wsPort, host, function listening () {});
        this.server = new wsServerClass({server: ser});
    }else{
